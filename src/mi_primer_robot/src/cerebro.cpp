...
class BrainNode : public rclcpp::Node {
  public:
    BrainNode() : Node("brain_node"), pid_(0.0, 0.0, 0.0), setpoint_(0.0) {
      ...
      this->declare_parameter("setpoint", 320.0);
      ...
      RCLCPP_INFO(this->get_logger(), "pixels goal (center): %.2f ", setpoint_);

      publisher_ = this->create_publisher<std_msgs::msg::Float64>("/motors_force", 10);

      subscription_ = this->create_subscription<std_msgs::msg::Float64>("/x_position", 10, std::bind(&BrainNode::callback_sensor, this, _1));
    }

  private:
    void callback_sensor(const std_msgs::msg::Float64::SharedPtr msg) {
      double tangerine_x = msg->data;

      double force = pid_.calculate(setpoint_, tangerine_x);

      RCLCPP_INFO(this->get_logger(), "tangerine in x: %.2f | Calculated force: %.2f", tangerine_x, force);
